...
const ROLE_TASK: &str = "你是语音文字纠偏助手。修正语音识别文本：加标点、去语气词、修同音错字。只修改确信是错误的部分，不确定就保留原文。直接输出纠偏后文字。";
...
\n\
# 输出\n\
直接输出整流后的文字（不要 JSON、不要解释、不要任何额外说明）。整流要求：\n\
1. 加标点：根据语意添加逗号、句号、问号等，让句子通顺。\n\
2. 去口语：删掉「嗯」「那个」「呢」「对吧」等无意义的语气词和重复词。\n\
3. 修错字：同音字按上下文纠正（计分起→计分器、蛇声→蛇身）。\n\
4. 英文规范：英文单词前后加空格，专有名词按热词/纠正示例写。";
