...
impl LargeDict {
    ...
    /// Also loads cached jianpin index if available (.jianpin file next to .fst).
    fn load_fst_file(&mut self, path: &str) -> std::io::Result<usize> {
        ...
        let jianpin_path = format!("{path}.jianpin");
        if self.try_load_jianpin_cache(&jianpin_path, path) {
            ...
            self.save_jianpin_cache(&jianpin_path);
            eprintln!("[swift-ime] large dict loaded (FST, built jianpin) in {}ms", elapsed.as_millis());
        }
    ...
    fn try_load_jianpin_cache(&mut self, cache_path: &str, fst_path: &str) -> bool {
    ...
    fn save_jianpin_cache(&self, cache_path: &str) {
        let mut buf = Vec::new();
