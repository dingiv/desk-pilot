...
pub struct EnglishWeights {
    pub exact: f64,
    /// W3(round10):exact 的词频质量系数 —— exact 分 = exact +
    /// exact_quality × frequency_band(词频)。高频英文词(make/nine,grade
    /// 顶档)压过拼音多字谐音词(马克 0.654),低频英文词(sou,band 0.35)
    /// 让位中文顶流(搜 0.647)。词频不参与时一条固定分数线决一切,两类
    /// 场景必错其一。
...
/// round10:16 → 64(top_n 默认 32,预过滤须宽于语义截断才有意义)。
const PREFILTER_TAKE: usize = 64;
...
impl EnglishFamily {
    ...
    fn apply_recency(&self, out: &mut [ScoredCandidate]) {
        ...
        for c in out.iter_mut() {
            // round21 单公式:g = GAIN_MAX × 2^(−age/半衰期),
            // score' = a + (1-a) × g(与拼音侧同一条公式,常量同源)。
    ...
    fn query_layer(
        ...
        // W3:exact 词频化 —— exact + exact_quality × band(词分)。
        // user 层整体乘 user_boost(学习语义保留)。
        let exact_base = if user_layer {
            w.exact * w.user_boost
        } else {
            w.exact
        };
        let exact_quality = if user_layer {
            w.exact_quality * w.user_boost
        } else {
            w.exact_quality
        };
        ...
        for (word, freq) in words[start..].iter() {
            ...
            // 短词/短输入降权(词典层):两字母输入几乎总是中文简拼,
            // 英文短词(exact)与短输入捞出的英文前缀长尾(cdc/cds)
            // 都不该压过它们。非 exact 才看输入长度 —— exact 已被词短
            // 条件覆盖,两字母 exact 即两字母词。
            let short = short_penalty
                && (word.chars().count() <= 2
...
impl CandidateFamily for EnglishFamily {
    ...
    fn top_n(&self) -> usize {
        // round10 放宽 8 → 32:长前缀补全(clea→clean/clear/clears/…)需要
        // 深池;可经 `weights.family_top_n.english` 覆盖。
        32
    }
...
mod tests {
    ...
    fn learned_short_words_not_penalized() {
        // 自生词(用户层)不降权:学入 "cd" 后仍以全权重出现(user 层词频
        // 10000 → band 0.9 → 0.88+0.08×0.9=0.952)。
        let fam = EnglishFamily::new(); // 空 base,只看 learned 层
        ...
        assert!((c.raw_score - 0.952).abs() < 1e-9, "learned short word keeps full weight");
    }
}
