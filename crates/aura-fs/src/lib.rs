...
//! ```rust
//! fn main() { aura_fs::emit_namespaces(); }
//! ```
//!
...
/// declared in `[package.metadata.aura-fs]` (requires the crate to have a `build.rs` that calls
...
macro_rules! loader {
    () => {{
        const __NS: &[(&str, &str, &str)] =
            include!(concat!(env!("OUT_DIR"), "/aura_fs_ns.rs"));
...
// ── build.rs helper: read Cargo.toml namespaces → generate aura_fs_ns.rs ──────

/// Call from a crate's `build.rs`: reads `[package.metadata.aura-fs]` from this crate's
/// `Cargo.toml`, generates `{OUT_DIR}/aura_fs_ns.rs` (a const table consumed by [`loader!`]).
///
...
pub fn emit_namespaces() {
    ...
    let mut code = String::from("// AUTO-GENERATED by aura_fs::emit_namespaces() — do not edit.\n&[\n");
    ...
    code.push_str("]\n");
    let dest = Path::new(&out_dir).join("aura_fs_ns.rs");
    std::fs::write(&dest, code)
        .unwrap_or_else(|e| panic!("write {}: {e}", dest.display()));
    println!("cargo:rerun-if-changed=Cargo.toml");
}

/// Parse `[package.metadata.aura-fs]` from Cargo.toml. Returns `Vec<(name, dev, prod)>`.
...
fn parse_metadata(cargo_toml: &Path) -> Vec<(String, String, String)> {
    ...
    // Find the [package.metadata.aura-fs] section.
    let marker = "[package.metadata.aura-fs]";
...
// ── aura-fs's own build.rs (generates its own empty namespace table) ──────────
...
mod tests {
    ...
    fn parse_metadata_no_section() {
        // A Cargo.toml without [package.metadata.aura-fs] → empty vec.
