...
### Recent member(近期指数权重合成,round20 连续化)

提交时记录词 + wall-clock 时间戳(round16 起由 MemoryLayer 的 recent 表
承载,round19 起时间统计 L1/L2 分表、tier 三级穿透)。round20 起
近期指数从五档阶梯(10s/1h/5h/1d/3d 同档同分)改为**连续指数衰减**:

```
b(t) = min(5, 5 × 2^(−t/半衰期) + (count/3).min(1))     t ≤ 3d,否则 0
       半衰期 = 18h(对齐旧阶梯中段:5h≈4.1、1d≈2.2)
```

- 同档窗口内几分钟之差也可分辨(旧阶梯同分);频次增强也从 ≥3 次
  阶跃 +1 连续化为 (count/3) 线性爬升;超 3 天惰性移出不变。
...
```
z = (1-a) × (a+b) / 8 + a        a = 候选原权重, b = 近期指数(连续 0..5)
```
...
固定不可配:recent 合成公式(z 公式与连续衰减半衰期 18h)、前缀/emoji
