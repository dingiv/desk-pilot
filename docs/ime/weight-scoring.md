...
### Recent member(近期增益,round21 单公式)

提交时记录词 + wall-clock 时间戳(round16 起由 MemoryLayer 的 recent 表
承载,round19 起时间统计 L1/L2 分表、增益查询三级穿透)。round21 把
旧三段链(五档阶梯 → b 空间 → z 合成)合并为**一个衰减因子 + 一条
合成公式**:

```
g(t) = 0.70 × exp2(−t / 18h)        t > 3d → 0(惰性移出时间表;频率表统计不受影响)
score' = a + (1-a) × g(t)           a = 候选原权重
```

- `GAIN_MAX=0.70` 对齐旧 z 公式 b=5 档的等效增益((a+5)/8 ≈ 0.67~0.74);
- 增量与 (1-a) 成比例 —— 低权重词获更大加成、高权重词增量趋零,
  天然 < 1 不顶满(三条旧性质全保留);
- **count 不双计**:频次增强只在 `effective_frequency`(词典域)一处,
  时间增益是纯时间函数;
- `exp2` 直取(基底 0.5 恒等式),免去 powf 内部 log2 换算;
- 超考3 天惰性移出**只删时间表条目**,overlay 频率统计不动(分表核心不变式)。
...
固定不可配:recent 单公式(GAIN_MAX/半衰期 18h)、前缀/emoji
距离衰减(0.85 底数)、emoji ≤2 字母降权、phrase count 曲线斜率
(0.02/次,起点 0.70)。
