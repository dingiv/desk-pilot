...
- **修复 CLI 展示模式污染账本**:show_candidates_with_async 末尾 Enter(raw
  ...
  同一教训)。
- **round23 续 — 完整命令形态**:菜单选档后结果视图回显完整命令
  `yibu'#freq/up/10000 · 30000 → 40000`(空格提交该词);该形态本身
  可直接输入(`#freq/down/1000` 带参路径,force_fire 沿用 #del 的
  触发即提交语义,操作对象 = 链式高亮锚点 —— force_fire 补锚点重排)。
- 验证:222 全绿;clippy 0;eval 持平 98.0/99.4。

## round 24 — #freq 真实效果修复(用户 jiekou 场景)

用户实测 `jiekou'#freq/down/50000` 后借口仍压接口,分数基本不动。三个
叠加根因,逐一修复:

1. **提交副作用淹没调整**(主凶):force_fire 的 fire-and-commit 语义把
   被调词顺手提交 → recency 增益 g≈0.70 + 有机增量当场抵消降幅
   (实测 down 后 0.545→0.717 不降反升)。#freq 改纯元命令:结果视图
   interactive,不提交文本(Esc 收尾)。
2. **负向下限太保守**:base×0.5 相对下限对多字词(手工等级 base 数千)
   只值 ~0.02 分。下限改绝对地板 1(降权要降得动;账本可 up 恢复)。
3. **log₂ 刻度下的步长失配**:freq_to_score 是对数刻度,种子词 base
   数万~数十万(借口实测 129,390),±50000 只值 ~0.02 分。步长档位
   重标 ±1k/1w/10w/100w,账本总上限 DELTA_CEIL 放宽 ±100 万。
   场景复验:down/100000 → 借口 0.545→0.495 沉到 #2,接口登顶;
